...
interface ExplorerCompletionProps {
  category: Category
...
/**
 * Pantalla final del nivel "Explorador de experiencias".
 *
 * Muestra el logro obtenido, el XP ganado y el progreso general ya
 * actualizado. El guardado en Supabase ocurre al entrar aquí, por eso
 * esta pantalla refleja el estado del guardado.
 */
export default function ExplorerCompletion({
  category,
  experience,
  xpEarned,
  isReplay,
  ...
  return (
    <div
      className="nc-completion page-fade"
      style={{
        '--route-color': category.color,
        '--route-bg': category.background,
      } as React.CSSProperties}
    >
      <div className="nc-completion__icon">🎉</div>

      <h1 className="nc-completion__title">¡EXPERIENCIA COMPLETADA!</h1>

      <p className="nc-completion__message">
        Te atreviste a explorar algo nuevo con tu tiempo libre. Cada experiencia
        que vives fuera de la pantalla suma a quien eres.
      </p>
      ...
      <p className="nc-completion__route">
        Tu experiencia: {experience.emoji} <strong>{experience.label}</strong> ·{' '}
      ...
      {saveFailed && (
        <div className="nc-completion__status nc-completion__status--error" role="alert">
          <p>
            ...
            inténtalo de nuevo: tu experiencia no se ha perdido.
          </p>
